...
public final static int MAXD=10;
    public static void main(String args[]) {
		...
		lector.nextLine();
		// a apartir de aqui comiensan las modificaciones del seguimiento 3
		System.out.println("un valor de daño para conocer todos los valores dentro del arreglo inferiores al daño :");
		double searchDamage = lector.nextInt();
		lector.nextLine();
		...
		double confirmDamage = inferiorDamage(specialDamage,searchDamage);
		System.out.println("los resultados son :"+confirmDamage);
		
		double totalDamage = sumDamage(specialDamage);
		System.out.println("la sumatoria total de los datos es :"+totalDamage);
		
		
		System.out.println("ingrese el danio que desea buscar :");
		double damageAremplasar = lector.nextDouble();
		
		System.out.println ("ingrese el danioo por el que desea remplasarlo");
		double damageDeRemplaso = lector.nextDouble();
		
		changeDamage(specialDamage, damageAremplasar, damageDeRemplaso);
		System.out.println("Arreglo especialDamage despues de la modificacion:");
		for (int i = 0; i < specialDamage.length; i++) {
		System.out.println("Elemento " + (i + 1) + ": " + specialDamage[i]);
		}
	...
	*/
    ...
    resistenciaAleatoria = Math.round(resistenciaAleatoria * 100.0) / 100.0; // Redondear a dos decimales
	...
	*/
	...
	/**
	* Calcula la cantidad de elementos en un arreglo que son inferiores a un valor dado.
	*
	* @param specialDamage Un arreglo de números en el que se buscarán los elementos inferiores.
	* @param searchDamage El valor de referencia para la comparación.
	* @return La cantidad de elementos en el arreglo que son menores que el valor de referencia.
	*/
    public static double inferiorDamage(double[] specialDamage,double searchDamage){
		double ifDamage=0;
		for (int i = 0; i < specialDamage.length ; i++){
			if(specialDamage[i]<searchDamage){
				ifDamage=ifDamage+1;
				System.out.println(specialDamage[i]);
	...
	/**
	* Calcula la suma de productos de cada par de elementos en un arreglo.
	*
	* @param specialDamage Un arreglo de números cuyos elementos se multiplicarán y sumarán.
	* @return La suma de los productos de cada par de elementos en el arreglo.
	*/
	
	public static double sumDamage(double[] specialDamage) {
    double sumatoriaDamage = 0;
    for (int i = 0; i < specialDamage.length; i++) {
        for (int j = 0; j < specialDamage.length; j++) {
            if (i != j) { 
                sumatoriaDamage += specialDamage[i] * specialDamage[j];
            }
        }
    }
    return sumatoriaDamage; 
	}
	/**
	* Cambia un valor en un arreglo por otro valor dado.
	*
	* @param specialDamage Un arreglo de números en el que se realizará el cambio.
	* @param damageAremplasar El valor que se buscará y reemplazará.
	* @param damageDeRemplaso El valor que se utilizará como reemplazo.
	*/
	
	public static void changeDamage(double[] specialDamage, double damageAremplasar, double damageDeRemplaso) {
    boolean found = false; // Variable para rastrear si se encontró el valor
    for (int i = 0; i < specialDamage.length; i++) {
        if (Math.abs(specialDamage[i] - damageAremplasar) < 1e-6) {
            specialDamage[i] = damageDeRemplaso;
            found = true; // Marcamos que se encontró el valor
            break; // Salimos del bucle una vez que se encuentra el valor
        }
    }
    
    if (!found) {
        System.out.println("Error: El valor " + damageAremplasar + " no se encuentra en el arreglo.");
    }
}
